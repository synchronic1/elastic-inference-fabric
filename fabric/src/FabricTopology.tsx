...
function Node({ node, active }: { node: FabricNode; active: boolean }) {
  ...
  return <article className={`topology-node ${node.status} ${active ? 'working' : ''}`} data-active={active || undefined}>
    <header><div><span className="topology-node-status" /><h3>{node.node_id}</h3></div><b>{node.status}</b></header>
...
export default function FabricTopology({ fabric, activeNodeId }: { fabric: FabricState; activeNodeId?: string }) {
  const runningNodeIds = new Set(fabric.jobs.filter((job) => job.status === 'running').map((job) => job.node_id));
  if (activeNodeId) runningNodeIds.add(activeNodeId);
  const active = fabric.nodes.some((node) => node.snapshot.active_requests > 0 || runningNodeIds.has(node.node_id));
  return <section className="topology" aria-labelledby="topology-heading">
    <header className="topology-heading"><div><p className="eyebrow">LIVE TOPOLOGY</p><h2 id="topology-heading">Request path to local inference</h2></div><p>Rates are last measured, not capacity guarantees.</p></header>
    <div className={`topology-map ${active ? 'has-work' : ''}`}>
      <div className="topology-origin"><span className="topology-icon">↗</span><div><b>Agent / harness</b><small>Authenticated request</small></div></div>
      <div className="topology-link request" aria-hidden="true"><i /> <span>Request</span></div>
      <div className="topology-router"><span className="topology-icon">⌘</span><div><b>Authenticated MCP router</b><small>Cloud control plane</small></div></div>
      <div className="topology-link request-node" aria-hidden="true"><i /> <span>Request → node</span></div>
      <div className="topology-nodes">{fabric.nodes.length ? fabric.nodes.map((node) => <Node key={node.node_id} node={node} active={node.status === 'online' && (node.snapshot.active_requests > 0 || runningNodeIds.has(node.node_id))} />) : <p className="topology-empty">No node heartbeat has been received.</p>}</div>
      <div className="topology-return-lane" aria-hidden="true"><span>← Heartbeat / Result</span></div>
    </div>
    <footer className="topology-legend"><span><i className="request" /> Request</span><span><i className="result" /> Result</span><span><i className="heartbeat" /> Heartbeat</span><span><i className="resident" /> Resident</span><span><i className="cached" /> Cached</span><span><i className="unprovisioned" /> Unprovisioned</span></footer>
  </section>;
}
