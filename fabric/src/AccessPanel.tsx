...
export function AccessIntroduction() {
  return <section className="access-panel access-introduction" aria-labelledby="access-setup-heading">
    <header className="access-heading"><div><p className="eyebrow">ACCESS TOKENS</p><h2 id="access-setup-heading">Get access to the fabric</h2></div><span className="access-role">Admin-issued</span></header>
    <div className="access-steps">
      <div><h3>1. Get your token</h3><p>Ask the fabric administrator for a read-only viewer token or an individual agent token. There is no public self-registration or anonymous token issuance.</p></div>
      <div><h3>2. Sign in</h3><p>Paste your token into <a href="#fabric-access">Sign in to Fabric</a>. Node connector tokens cannot sign in to the dashboard.</p></div>
      <div><h3>3. Explore or connect</h3><p>Viewer tokens show live resources only. Agent tokens can connect to <code>/mcp</code> using Bearer authentication. Only administrators can issue or revoke tokens in this panel after signing in.</p></div>
    </div>
    <details className="access-owner-help"><summary>Project owner: first administrator sign-in</summary><p>From this project on the configured Mac, run <code>bin/fabric copy-token</code> to copy the locally stored bootstrap administrator token, then paste it into the sign-in form. Clear the clipboard afterward and never share this administrator credential with agents.</p><p>Once signed in, use <strong>Issue a Fabric access token</strong> below to create a separate agent or node credential with its own expiry. On another deployment, obtain administrator access from its operator.</p></details>
  </section>;
}
...
export default function AccessPanel() {
  ...
  return <section className="access-panel" aria-labelledby="access-heading">
    <header className="access-heading"><div><p className="eyebrow">ACCESS &amp; MCP</p><h2 id="access-heading">Access tokens</h2></div><span className={`access-role ${principal.role}`}>{principal.role}</span></header>
    {!isAdmin && <p className="access-policy">Your {principal.role} token cannot issue or revoke credentials. Ask an administrator for a new token or a change in access.</p>}
    ...
    {principal.role === 'viewer' ? <p className="access-policy">Read-only live dashboard and resource API access. Workload submission, private job results, node connections, and MCP are disabled for viewers.</p> : <div className="mcp-guide"><div><h3>MCP endpoint <code>{window.location.origin}/mcp</code></h3>{principal.role === 'node' ? <p>Node access is for the connector only; it cannot use MCP tools.</p> : <p>Streamable HTTP with a preprovisioned access token — not OAuth. Send <code>Authorization: Bearer &lt;FABRIC_ACCESS_TOKEN&gt;</code>.</p>}<p className="tool-list">Tools: <code>fabric_resources</code>, <code>fabric_models</code>, <code>fabric_submit_task</code>, <code>fabric_get_task</code></p></div><a href="/openapi.json">MCP / API docs ↗</a></div>}
    {notice && <p className="access-error" role="status">{notice}</p>}
    {isAdmin && <div className="admin-access"><div><p className="eyebrow">TOKEN ADMINISTRATION</p><h3>Issue a Fabric access token</h3><p className="access-policy">Viewer tokens inspect live resources only. Create one credential per agent or node. Node tokens are bound to a node ID; administrator tokens can manage all access.</p></div><form className="token-form" aria-label="Issue a Fabric access token" onSubmit={create}><label>Label<input value={form.label} maxLength={80} onChange={(e) => setForm({ ...form, label: e.target.value })} placeholder="e.g. research-agent" required /></label><label>Role<select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value as FabricRole })}><option value="viewer">Viewer (read-only)</option><option value="agent">Agent</option><option value="node">Node connector</option><option value="admin">Admin</option></select></label>{form.role === 'node' && <label>Node ID<input value={form.node_id} onChange={(e) => setForm({ ...form, node_id: e.target.value })} required /></label>}<label>Expires in days<input type="number" min="1" max="365" value={form.expires_in_days} onChange={(e) => setForm({ ...form, expires_in_days: e.target.value })} required /></label><button disabled={busy}>{busy ? 'Working…' : 'Issue token'}</button></form>
