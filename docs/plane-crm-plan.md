...
11. **`@plane/constants` and `@plane/i18n` ship prebuilt `dist/` bundles** (`main`/`module`
    in `package.json` point at `dist/index.{mjs,js}`, generated by `tsdown`) — `pnpm --filter
    web build` does **not** rebuild them. Editing `packages/constants/src/workspace.ts` (the
    sidebar registry) or `packages/i18n/src/locales/en/core.ts` and then running
    `~/plane-deploy.sh web` ships a bundle with the *old* sidebar/labels, no error, no warning.
    Hit live 2026-09-22: the CRM route worked but the sidebar nav item silently didn't appear
    until `pnpm --filter @plane/constants build && pnpm --filter @plane/i18n build` ran first.
    **Any edit under `packages/*/src/` needs an explicit `pnpm --filter @plane/<pkg> build`
    before `plane-deploy.sh web`.**
12. **Unmanaged-model fields don't type-coerce on `.create()`/`.save()`.** A real Django model
    field (migrated, going through a form/serializer) casts a request string to `date`/`int`/etc
    automatically; a plain `models.Model(managed=False)` instance assigned a raw string from
    `request.data` keeps it as a string in memory — `.create()` doesn't run `full_clean()`.
    `CrmOpportunity.close_date` held a `str` until the first `.isoformat()` call crashed it
